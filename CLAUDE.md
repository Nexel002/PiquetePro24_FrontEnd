...
- Não commitar `.env`.
- **Sempre que for implementado algo que não está no TRD nem no Plano de Implementação** (pedido do utilizador fora do escopo já documentado, ideia surgida a meio da implementação, correção que introduz comportamento novo) **tem de ser escrito nos dois documentos antes de a funcionalidade ser dada como concluída**: um novo item/Adendo no TRD (`docs/TRD_v1.1_Marketplace_Servicos_Locais_PWA.md`) descrevendo contexto e decisão, e um critério de entrega correspondente no `docs/PLANO_IMPLEMENTACAO_FRONTEND.md`, marcado quando validado. Aplica-se em espelho ao repositório `PiquetePro24_Backend` sempre que a funcionalidade tiver componente de backend (`Doc's/TRD_v1.1_Marketplace_Servicos_Locais_PWA.md` e `Doc's/PLANO_IMPLEMENTACAO_BACKEND.md`) — os dois TRDs são cópias sincronizadas, nunca deixar um repositório com a versão desatualizada.
