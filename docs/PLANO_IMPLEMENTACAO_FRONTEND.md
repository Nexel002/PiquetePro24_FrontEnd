...
- [ ] **(TRD Adendo v1.4, item D)** Apagar conta: secção "Zona de perigo" em `Profile.tsx`, confirmação em duas etapas, `useDeleteAccount()` chama `DELETE /profile` e depois `supabase.auth.signOut()` + `queryClient.clear()`, redireciona para `/`. **Não testado de ponta a ponta** — depende da migration de CASCADE do backend (`20260916131336_cascade_eliminacao_conta.sql`), ainda não aplicada no Supabase real.
