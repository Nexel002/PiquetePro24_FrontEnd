...
- [ ] **(TRD Adendo v1.7)** Chamar `POST /notifications/welcome` logo a seguir a um `supabase.auth.signUp()` com sessão imediata. **Pendente — nada implementado.** Identificado ao perguntar explicitamente pelo estado do frontend depois da entrega do backend, não durante a implementação (desvio à regra da Secção 1 deste CLAUDE.md, corrigido agora só como registo — a implementação continua por fazer).
- [ ] **(TRD Adendo v1.7)** Ecrã "Esqueci a password" em `Login.tsx` (só email, chama `POST /auth/password-recovery`, mostra sempre a mensagem genérica do backend) + nova rota `/definir-nova-password` (lê a sessão de recuperação via `detectSessionInUrl`, formulário de nova password, `supabase.auth.updateUser({ password })`). **Pendente — nada implementado**, incluindo a rota em si, que hoje não existe: o link de recuperação enviado pelo backend não tem para onde ir.
...
- [ ] **(TRD Adendo v1.8)** Tela de administração para o audit log (`GET /admin/audit-log`, filtros por utilizador/ação/tipo de entidade/data/sucesso), mesma convenção de acesso do item acima (`profile.role === 'ADMIN'`). **Pendente — nada implementado.** Mesma situação em que `/admin/kyc` esteve antes do Adendo v1.6: o endpoint existe no backend, sem nenhuma tela a consumi-lo.
