...
**Nome de lugar em vez de coordenadas (feedback do utilizador ao testar a tela de perfil):** a captura de GPS mostrava latitude/longitude brutos ("-25.9655, 32.5832"), pouco legível. Adiciona `services/geocoding.ts` + `hooks/useReverseGeocode.ts` (TanStack Query, `staleTime: Infinity` — o nome de um lugar não muda) a consumir o novo `GET /geocode/reverse` do backend, tanto na "Localização encontrada" (GPS antes de confirmar) como na "Localização atual" (GPS já gravado, via `latitude`/`longitude` que `GET /profile` agora também devolve). Cai de volta para as coordenadas cruas se a geocodificação falhar (nunca bloqueia o fluxo de confirmar localização). O backend evoluiu de Geocoding API para Places API (Nearby Search) durante esta validação — ver decisão no plano do backend — porque a primeira devolvia Plus Codes (`5HG9+X9, Maputo`) em vez de endereços de rua em zonas menos mapeadas.
