...
- Notificações in-app (toast) para mudanças de estado. **Não implementado** — mensagens de sucesso/erro são inline (mesmo padrão já usado em `LocationForm.tsx`/`Profile.tsx`), não um sistema de toast dedicado.
